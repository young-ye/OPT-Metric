...
def opt(G_gt_path, G_pred_path, num, save_path):
    start = time.time()
...
def main():
    ...
    for num_false in num_false_set:
        cores = int(multiprocessing.cpu_count() * 0.4)  # multiprocessing.cpu_count()
        print(cores)
        pool = multiprocessing.Pool(processes=cores)

        save_path = rf'G:\result\metric9\10000\opt\miss\miss_{num_false}_result.txt'  # 存放记录的文件
        gt_dir = r'G:\data_newmetric\opt\metric9\10000\gt'
        pre_dir = rf'G:\data_newmetric\opt\metric9\10000\miss\{num_false}'  # 金标准
        res = []

        # for G_gt_path in glob.glob(os.path.join(gt_dir, '*.swc')):  # 读所有的swc文件
        #     num = os.path.splitext(os.path.split(gt_swc_path)[-1])[0]
        #     print('fname的值为：{}'.format(num))
        #     out_swc_name = f'{fname}.swc'
        #     print(out_swc_name)
        ...
        for G_pred_path in tmp_list:  # 读所有的swc文件
            ...
            # opt(G_gt_path, G_pred_path, num, save_path)

            res.append(pool.apply_async(opt, args=(G_gt_path, G_pred_path, num, save_path)))
